...
namespace LiveSplit.Memory {
    ...
    public struct MemInfo {
        ...
        public uint Type;
        public override string ToString() {
            return $"{(ulong)BaseAddress:X} {Protect:X} {State:X} {Type:X} {(ulong)RegionSize:X}";
        }
    }
    public class MemorySearcher {
        ...
        public IntPtr FilterMemory(Process process) {
            GetMemoryInfo(process.Handle);
            return memoryInfo.Count > 0 ? memoryInfo[0].BaseAddress : IntPtr.Zero;
        }
